...
- **Git initialized** — remote `monosolusi/queue-flow`, default branch `main`.
  ...
  **re-call to the same counter** (the ticket retains its `_counterId` from the
  ...
  is cautious about. **Caller side-effect (open follow-up):** a recalled ticket
  now re-shows in the caller `active` list (an improvement — today the
  `STATUS_UPDATED` `idx === -1` guard dropped it, so a recalled ticket was
  invisible to staff) but with `categoryId: ''` since it is in neither `waiting`
  nor `active`, degrading the QUE-20 transfer chooser's "exclude current
  category" for a recalled ticket. QUE-5 (PR #30) scoped the *transfer*
  active-eviction and did not address this recall `categoryId` recovery, so it
  remains an open caller follow-up (not blocking QUE-4 — the TV, the QUE-4
  scope, is fully fixed).
