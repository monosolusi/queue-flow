 ...
 *
 * Root cause (verified in the @xyflow/react source `XYHandle.isValidHandle`): in
 * `ConnectionMode.Strict` (the React Flow default), a connection that STARTS at
 * a `source` handle is only valid when it ENDS on a `target`-TYPED handle
 * (`(isTarget && handleType === 'source') || (!isTarget && handleType === 'target')`).
 * Each node side carries one `source` + one `target` handle, and React Flow
 * drops onto the handle under the cursor. When that nearest handle was a
 * `source`, the drop was rejected — so a side read unconnectable whenever its
 * `source` handle was the nearer of the pair, even though nothing was connected
 * there yet.
 *
 ...
 * source-start drag may land on a `source` handle too, and every side accepts a
 * drop. Arrow direction is preserved because reversal is keyed on the START
 * handle's TYPE (not connection mode), and `StateNode` keeps target handles
 * drop-only via `isConnectableStart={false}` (pinned by the sibling
 * `StateMachineWorkflow.test.tsx` "makes target handles drop-only" test).
 *
