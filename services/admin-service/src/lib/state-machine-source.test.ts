...
function defaultForm(): StateMachineForm {
  return {
    ...
    transitions: DEFAULT_STATE_MACHINE.transitions.map((t) => ({
      ...t,
      requeuePolicy: t.requeuePolicy ?? { kind: 'KEEP' },
    })),
    positions: {},
    nodeActions: {},
    descriptions: {},
...
function richForm(): StateMachineForm {
  return {
    ...
    transitions: [
      { from: 'WAITING', to: 'VERIFIKASI', actionLabel: 'Panggil Berikutnya', requeuePolicy: { kind: 'KEEP' } },
      { from: 'VERIFIKASI', to: 'SERVING', actionLabel: 'Mulai Melayani', requeuePolicy: { kind: 'KEEP' } },
      {
        from: 'VERIFIKASI',
        to: 'WAITING',
        actionLabel: 'Kembalikan ke Antrian', requeuePolicy: { kind: 'KEEP' },
        sourceSide: 'bottom',
        targetSide: 'top',
      },
...
function selfLoopForm(): StateMachineForm {
  return {
    ...
    transitions: [
      ...
      { from: 'CALLING', to: 'CALLING', actionLabel: 'Panggil Ulang', requeuePolicy: { kind: 'KEEP' } },
      { from: 'CALLING', to: 'COMPLETED', actionLabel: 'Selesai Layan', requeuePolicy: { kind: 'KEEP' } },    ],
    positions: {},
    nodeActions: {},
    descriptions: {},
    endSources: [],
...
function singleStateForm(): StateMachineForm {
  return {
    mode: 'custom',
    states: ['WAITING'],
    transitions: [{ from: 'WAITING', to: 'WAITING', actionLabel: 'Panggil Ulang', requeuePolicy: { kind: 'KEEP' } }],    positions: {},
    nodeActions: {},
    descriptions: {},
    endSources: [],
    terminalNodes: { start: 'auto', end: 'auto' },
...
describe('formToXml — Kaleo workflow-definition shape', () => {
  ...
  it('a self-loop on a sink keeps it a <state> (the diagram and the source agree on terminal)', () => {
    ...
    const form: StateMachineForm = {
      ...
      states: ['A', 'S'],
      transitions: [
        { from: 'A', to: 'S', actionLabel: 'Lanjut', requeuePolicy: { kind: 'KEEP' } },
        { from: 'S', to: 'S', actionLabel: 'Ulangi', requeuePolicy: { kind: 'KEEP' } },      ],
    };
  ...
  it('derives a Kaleo <name> slug from the action label, deduped within the source node', () => {
    ...
    const form: StateMachineForm = {
      ...
      states: ['A', 'B', 'C'],
      transitions: [
        { from: 'A', to: 'B', actionLabel: 'Lanjut', requeuePolicy: { kind: 'KEEP' } },
        { from: 'A', to: 'C', actionLabel: 'Lanjut', requeuePolicy: { kind: 'KEEP' } },      ],
    };
    const xml = formToXml(form);
  ...
  it('falls back to a placeholder slug when a label has nothing sluggable', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: '→ ✓', requeuePolicy: { kind: 'KEEP' } }],    };
...
describe('formToXml — positions in node <metadata>', () => {
  it('each node carries xy from form.positions', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: 'go', requeuePolicy: { kind: 'KEEP' } }],      positions: { A: { x: 10, y: 20 }, B: { x: 240, y: 0 } },
    };
    const xml = formToXml(form);
  ...
  it('defaults absent positions to the deterministic autoLayout (matching the diagram)', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: 'go', requeuePolicy: { kind: 'KEEP' } }],      positions: {},
    };
    const xml = formToXml(form);
    ...
    // of truth).
    const auto = autoLayout(['A', 'B'], [{ from: 'A', to: 'B', actionLabel: 'go', requeuePolicy: { kind: 'KEEP' } }]);
  ...
  it('the XML positions match the Diagram autoLayout for the default graph (single source of truth)', () => {
    ...
    const xml = formToXml(form);
    const auto = autoLayout(DEFAULT_STATE_MACHINE.states, DEFAULT_STATE_MACHINE.transitions.map((t) => ({ ...t, requeuePolicy: t.requeuePolicy ?? { kind: 'KEEP' } })));
...
describe('formToXml — sparse metadata', () => {
  ...
  it('emits a node description only for non-empty overrides (sparse serialization)', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: 'go', requeuePolicy: { kind: 'KEEP' } }],      positions: { A: { x: 10, y: 20 }, B: { x: 240, y: 80 } },
...
describe('formToXml — node actions', () => {
  ...
  it('dedupes derived action names within a node', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: 'go', requeuePolicy: { kind: 'KEEP' } }],      nodeActions: {
        A: [
...
describe('formToXml — escaping', () => {
  it('escapes XML-special characters in element text', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A&B'],
      transitions: [{ from: 'A&B', to: 'A&B', actionLabel: 'go "there" <now>', requeuePolicy: { kind: 'KEEP' } }],    };
    const xml = formToXml(form);
  ...
  it('a CDATA payload can never contain the ]]> terminator', () => {
    ...
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: 'go', requeuePolicy: { kind: 'KEEP' } }],      descriptions: { A: 'akhiri dengan ]]> lalu lanjut' },
    };
    const xml = formToXml(form);
...
describe('round-trip (formToXml → xmlToForm)', () => {
  ...
  it('forces mode to custom and preserves the default graph structure', () => {
    ...
    // Same edges, regrouped under their source status by the nesting. The
    // round-trip resolves the wire's sparse requeuePolicy to a concrete KEEP,
    // so the expected edges carry the default policy explicitly.
    expect([...result.form.transitions].sort((a, b) => `${a.from}${a.to}`.localeCompare(`${b.from}${b.to}`))).toEqual(
      [...DEFAULT_STATE_MACHINE.transitions]
        .map((t) => ({ from: t.from, to: t.to, actionLabel: t.actionLabel, requeuePolicy: t.requeuePolicy ?? { kind: 'KEEP' } as const }))
  ...
  it('round-trips a non-default-routed edge (sourceSide/targetSide)', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: 'up', requeuePolicy: { kind: 'KEEP' }, sourceSide: 'bottom', targetSide: 'top' }],    };
    const result = xmlToForm(formToXml(form));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it('round-trips a description override and leaves an un-overridden status absent', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: 'go', requeuePolicy: { kind: 'KEEP' } }],      descriptions: { A: 'Antrian dimulai di sini' },
    };
    const result = xmlToForm(formToXml(form));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it('the parsed form passes validateCustomStateMachine and toStateMachineDto unchanged', () => {
    ...
    // transitions carry only `{ from, to, actionLabel }` even though the form
    // transitions carry the resolved `requeuePolicy: { kind: 'KEEP' }`.
