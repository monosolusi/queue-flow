...
function richForm(): StateMachineForm {
  return {
    ...
    transitions: [
      { from: 'WAITING', to: 'VERIFIKASI', actionLabel: 'Panggil Berikutnya' },
      { from: 'VERIFIKASI', to: 'SERVING', actionLabel: 'Mulai Melayani' },
      {
        from: 'VERIFIKASI',
        to: 'WAITING',
...
function selfLoopForm(): StateMachineForm {
  return {
    ...
    transitions: [
      ...
      { from: 'CALLING', to: 'CALLING', actionLabel: 'Panggil Ulang' },
      { from: 'CALLING', to: 'COMPLETED', actionLabel: 'Selesai Layan' },
    ],
    positions: {},
    nodeActions: {},
    descriptions: {},
...
function singleStateForm(): StateMachineForm {
  return {
    mode: 'custom',
    states: ['WAITING'],
    transitions: [{ from: 'WAITING', to: 'WAITING', actionLabel: 'Panggil Ulang' }],
    positions: {},
    nodeActions: {},
    descriptions: {},
    endSources: [],
...
function canonical(form: StateMachineForm): StateMachineForm {
  ...
  return {
    ...
    mode: 'custom',
    positions,
    transitions: form.states.flatMap((s) => form.transitions.filter((t) => t.from === s)),
    terminalNodes: {
...
describe('formToXml — Kaleo workflow-definition shape', () => {
  ...
  it('a self-loop on a sink keeps it a <state> (the diagram and the source agree on terminal)', () => {
    ...
    const form: StateMachineForm = {
      ...
      states: ['A', 'S'],
      transitions: [
        { from: 'A', to: 'S', actionLabel: 'Lanjut' },
        { from: 'S', to: 'S', actionLabel: 'Ulangi' },
      ],
    };
  ...
  it('derives a Kaleo <name> slug from the action label, deduped within the source node', () => {
    ...
    const form: StateMachineForm = {
      ...
      states: ['A', 'B', 'C'],
      transitions: [
        { from: 'A', to: 'B', actionLabel: 'Lanjut' },
        { from: 'A', to: 'C', actionLabel: 'Lanjut' },
      ],
    };
    const xml = formToXml(form);
  ...
  it('falls back to a placeholder slug when a label has nothing sluggable', () => {
    const form: StateMachineForm = {
      ...
      mode: 'custom',
      states: ['A', 'B'],
      transitions: [{ from: 'A', to: 'B', actionLabel: '→ ✓' }],
    };
...
describe('formToXml — <initial> derivation', () => {
  ...
  it('marks EVERY entry status when a graph has more than one (deviates from Kaleo on purpose)', () => {
    ...
    const form: StateMachineForm = {
      ...
      transitions: [
        { from: 'ANTRI_A', to: 'SELESAI', actionLabel: 'Selesai Layan' },
        { from: 'ANTRI_B', to: 'SELESAI', actionLabel: 'Selesai Layan' },
      ],
    };
    const xml = formToXml(form);
  ...
  it('a self-loop does not stop its status from being the entry point', () => {
    ...
    const form: StateMachineForm = {
      ...selfLoopForm(),
      transitions: [
        { from: 'WAITING', to: 'WAITING', actionLabel: 'Tunggu Lagi' },
        { from: 'WAITING', to: 'CALLING', actionLabel: 'Panggil Berikutnya' },
        { from: 'CALLING', to: 'COMPLETED', actionLabel: 'Selesai Layan' },
      ],
    };
...
describe('xmlToForm — lenient reading', () => {
  ...
  it('ignores the derived <initial>, <default>, transition <name> and node <labels>', () => {
    ...
    if (!result.ok) return;
    expect(result.form.states).toEqual(['A', 'B']);
    expect(result.form.transitions).toEqual([{ from: 'A', to: 'B', actionLabel: 'go' }]);
