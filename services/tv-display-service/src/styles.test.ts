...
/**
 * `css` with comments stripped. Use this for NEGATIVE assertions ("the file must
 * not contain X"): explanatory prose legitimately names the patterns it is
 * explaining why we avoid, so a bare `not.toContain` against the raw file fails
 * on a comment while the rules are perfectly correct. Same reasoning as the
 * `declsOnly` guard on the token rename below.
 */
...
describe('styles.css AC guards', () => {
  ...
  it('AC6: the active board is the single always-visible layer (no standby/crossfade)', () => {
    ...
    // Comment-stripped: the audio-unlock overlay's own comment explains WHY it
    // avoids a --hidden modifier and an opacity transition, so the raw file
    // legitimately contains those words in prose.
    expect(cssDecls).not.toContain('.standby');
    expect(cssDecls).not.toContain('--hidden');
    expect(cssDecls).not.toContain('position: absolute');
    expect(cssDecls).not.toContain('transition: opacity');
  ...
  it('audio-unlock overlay is a fixed full-screen layer using the shared focus baseline', () => {
    // position: fixed (not absolute) and conditional mounting (not a --hidden
    // modifier) are forced by the AC6 negations above, so this is the positive
    // half of that contract.
    const overlay = rule('.tv-audio-unlock');
    expect(overlay).toContain('position: fixed');
    expect(overlay).toContain('inset: 0');
    // Opaque background: a translucent scrim would leave the board competing
    // with the one instruction that matters on a wall-mounted screen.
    expect(overlay).toContain('background: var(--bg)');
    // No opacity animation (AC6) and no bespoke focus ring — the ring comes from
    // the shared _interactions.css baseline.
    expect(overlay).not.toContain('outline');
  });

  it('imports the shared a11y/interaction baseline now that the board has a control', () => {
    // The overlay is a real <button> — the first interactive element on the TV.
    // The header comment used to say "if a control is ever added, add the
    // interactions import then"; this guards that it actually happened, so
    // AC2/AC3/AC4 hold for real rather than vacuously.
    expect(css).toContain("@import './styles/_interactions.css'");
  });
