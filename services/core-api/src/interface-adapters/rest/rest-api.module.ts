...
/**
 * Wires the read-only REST surface for the caller workspace (QUE-19). The use
 * cases are pure, framework-free classes (no `@Injectable`/`@Inject` — they stay
 * decoupled from Nest, consistent with the existing application layer), so they
 * are provided here via factories that receive the repository ports injected
 * from {@link PersistenceModule}. Controllers depend on the use-case class
 * tokens, which Nest resolves because each is `provide`d as itself.
 */
@Module({
  imports: [PersistenceModule],
  controllers: [CountersController, QueueController],
  providers: [
