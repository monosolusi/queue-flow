 ...
 * NOT socket.io — core-api uses `@nestjs/platform-ws` (raw `ws`), so this speaks
 * plain WebSocket. Auto-reconnects with exponential backoff so the workspace
 * recovers when the local server restarts (offline resilience). The socket is
 * dumb: it only parses envelopes and forwards them; all projection logic lives
 * in the queue-store.
 */
